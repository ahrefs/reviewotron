...
verified file/field in `/home/me/code/opensource/reviewotron`. Protection set = 36 loved+valid findings
(`loved_profile`); `eval.jsonl` = 65 `must_not_flag` + 36 `must_keep_flagging`. Stage 4 replay = design doc
`docs/plans/2026-07-17-feedback-insights-loop-design.md` §7d.
...
- **Recall risk:** none directly (more context cannot suppress a finding); the risk is **cost**, not recall.
  Security's $345.90 tool-loop spend (INSIGHTS §9) is the warning: unbounded fetching is expensive. Cap it.
...
- **Security plugin cost/value.** `cost_efficiency`: security spent **$345.90** for **5** in-sample valid
  comments and **0** loved (vs general's $115.97 → 79 valid / 23 loved). Decide whether to keep the security
...
- **Per-repo `ignored_paths` / `ignored_file_regexes`.** 22 escalation path requests were unresolvable
  (guessed paths); the `apiv3_*` and `ahkit`/`keywords-explorer` clusters (`escalation.top_requested_paths`)
...
Every candidate change (Ranks 1–6) MUST be replay-tested against `eval.jsonl` per design doc
`docs/plans/2026-07-17-feedback-insights-loop-design.md` §7d (Stage 4, counterfactual replay) **before
